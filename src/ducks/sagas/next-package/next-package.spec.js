      ...
      const packageString = `
{
  ...
  "devDependencies": {
    "devDep": "prevVersion"
      ...
      expect(saga.next().value).toEqual(put(changePackageObject({ name: 'test' })));
      ...
      expect(saga.next({ success: true, version: 'nextVersion' }).value).toEqual(
        put(changeDependenciesNextVersion('dep', 'nextVersion')),
      );
      expect(saga.next().value).toEqual(put(changeDependenciesLoading('dep', false)));

      // devDependencies 조회
      expect(saga.next().value).toEqual(call(getNpmPackageVersion, 'devDep'));
      expect(saga.next({ success: true, version: 'nextVersion' }).value).toEqual(
        put(changeDevDependenciesNextVersion('devDep', 'nextVersion')),
      );
      expect(saga.next().value).toEqual(put(changeDevDependenciesLoading('devDep', false)));
